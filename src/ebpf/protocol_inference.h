...
//	bpf_printk("infer_http_message  \n");
	...
	if (count < 14) {
		bpf_printk("infer_http_message, MSG_UNKNOWN \n");
		return MSG_UNKNOWN;
	}

	if (is_http_response(buf)) {
		bpf_printk("infer_http_message, MSG_RESPONSE \n");
		return MSG_RESPONSE;
	}

	if (is_http_request(buf, count)) {
		bpf_printk("infer_http_message, MSG_REQUEST \n");
		return MSG_REQUEST;
	}
